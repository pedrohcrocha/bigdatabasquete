...
shinyUI(pageWithSidebar(
    ...
    # SIDEBAR PANEL
    sidebarPanel(
        conditionalPanel(condition="input.tabselected==1", h4("Big Data Basquete")
                         ),
        conditionalPanel(condition="input.tabselected==2",
                        selectInput("dataset", "Selecione uma temporada", choices=c("temporada_2018","temporada_2017","temporada_2016")),
                        uiOutput('team'),
                        radioButtons("choice","Escolha uma opção", choices=c("Dados" = 1, "Resumo" = 2 ))
                        ),
        conditionalPanel(condition="input.tabselected==3", "A seleção das opções 'Jogador' e 'Equipe' resultará em um cluster de dados.",
                         selectInput("count", "Selecione uma temporada", choices=c("temporada_2018","temporada_2017","temporada_2016")),
                         uiOutput("varx"),
                         uiOutput("vary")
                         ),
        conditionalPanel(condition="input.tabselected==4", 
                        selectInput("aux", "Selecione uma temporada", choices=c("temporada_2018","temporada_2017","temporada_2016")),
                        uiOutput("stat")
                        ),
        conditionalPanel(condition="input.tabselected==5", 
                         selectInput("adv", "Selecione uma temporada", choices=c("temporada_2018","temporada_2017","temporada_2016")),
                         uiOutput("advance")
        ),
    ...
    mainPanel(
        ...
        tabsetPanel(
            ...
            tabPanel("Avançadas", value=4, tableOutput("sadv")),
            id = "tabselected"
            ) # tabset Panel
        ) # Main Panel
    ) #pagewithSidebar
